...
class SLSTM(nn.Module):
    ...
    def forward(self, data, mask):
        ...
        def update_g_node(nhs, ncs, gh, gc, mask):
            ...
            xxx = torch.cat([gh[:, None, :].expand(B, L + 1, H), ihs], 2)
        ...
        print("emb1",embs.size())
        embs = self.input(embs)
        print("emb2",embs.size())
        # nhs = ncs = TZ(B,L,H)
        nhs = ncs = embs
        # gh = gc = TZ(B,H)
...
class BSLSTM(nn.Module):
    def __init__(self, nemb, nhid, num_layer, hyper=False, dropout=0.5, return_all=True):
        super(BSLSTM, self).__init__()
        if hyper:
            self.n_fc = nn.Linear(4 * nhid + nemb, 2 * nhid)
            self.n_h_fc = HyperLinear(4 * nhid + nemb, 5 * nhid)
            self.g_out_fc = nn.Linear(2 * nhid, nhid)
